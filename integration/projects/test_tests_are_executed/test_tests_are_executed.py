...
def test_valid_tests_are_run(dry_run_result: CompletedDryRun) -> None:
...
def test_invalid_tests_fail(dry_run_result: CompletedDryRun) -> None:
    report = assert_report_produced(dry_run_result)
    assert_node_failed(report, "test.test_tests_are_executed.first_layer_invalid_test")


def test_tests_for_missing_models_are_not_executed(
