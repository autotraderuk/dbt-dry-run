...
def test_linted_model_fails(dry_run_result: CompletedDryRun) -> None:
    node = get_report_node_by_id(
        dry_run_result.get_report(), "model.test_column_linting.badly_documented_model"
    )
    expected_errors = {
...
def test_linting_disabled_model_skipped(dry_run_result: CompletedDryRun) -> None:
    node = get_report_node_by_id(
        dry_run_result.get_report(), "model.test_column_linting.model_linting_disabled"
    )
...
def test_linting_not_defined_model_skipped(dry_run_result: CompletedDryRun) -> None:
    node = get_report_node_by_id(
...
def test_linting_model_with_structs_success(dry_run_result: CompletedDryRun) -> None:
    node = get_report_node_by_id(
        dry_run_result.get_report(), "model.test_column_linting.model_with_struct"
    )
    assert node.linting_status == LintingStatus.SUCCESS


def test_linting_enabled_in_model_in_sub_dir(dry_run_result: CompletedDryRun) -> None:
    node = get_report_node_by_id(
        dry_run_result.get_report(),
